...
class ASPP(SparseModule):
    def __init__(self,in_channels, out_channels, norm_fn, block_reps=2, indice_key=6, rate = [1,6, 12, 18]):
        ...
        self.aspp_block1 = Custom1x1Subm3d(out_channels, out_channels, 1, indice_key='bb_subm{}'.format(indice_key))
    ...
    def forward(self, input):
        x0 = self.aspp_block1(input)
        x1 = self.aspp_block6(input)
        x2 = self.aspp_block12(input)
        x3 = self.aspp_block18(input)
        
        out_feats = torch.cat((x1.features, x2.features,x3.features,x0.features), dim=1)
        x3 = x3.replace_feature(out_feats)
        out = self.conv_1x1(x3)
        return out
