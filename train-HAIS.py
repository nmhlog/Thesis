...
def train(epoch, model, optimizer, scaler, train_loader, cfg, logger, writer):
    ...
    for i, batch in enumerate(train_loader, start=1):
        ...
        with torch.cuda.amp.autocast(enabled=cfg.fp16):
            loss, log_vars = model(batch,current_epoch=epoch, return_loss=True)
...
def validate(epoch, model, val_loader, cfg, logger, writer):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            result = model(batch,current_epoch=epoch)
...
def main():
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if not args.skip_validate and (is_multiple(epoch, cfg.save_freq) or is_power2(epoch)) and epoch>20:
